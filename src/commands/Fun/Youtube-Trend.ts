...
export default class Command extends BaseCommand {
    constructor(client: WAClient, handler: MessageHandler) {
        super(client, handler, {
            command: '➣ ʏᴏᴜᴛᴜʙᴇ-ᴛʀᴇɴᴅ',
            aliases: ['youtube-trend','ytt'],
            description: 'Gives you the trending list of yt ',
            category: 'fun',
            
            usage: `${client.config.prefix}youtube-trend [title]`
        })
    }
    // static count = 0
    run = async (M: ISimplifiedMessage, { joined }: IParsedArgs): Promise<void> => {
        ...
        console.log(ethan)
        const { data } = await axios.get(`https://api-xcoders.xyz/api/info/trend/youtube?country=${ethan}&apikey=Zl0clXuAbx`)
